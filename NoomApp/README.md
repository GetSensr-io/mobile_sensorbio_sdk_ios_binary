...
- **`NoomApp/Info.plist`** — BLE permission strings + `bluetooth-central` background mode.
- **`NoomApp/NoomAppApp.swift`** — `@main` with the required init pattern: `SensorBioSDK.environment` → `bootstrapKeychain()` → `runDefaultsMigratorIfNeeded()` → `registerBGTasks()`.
- **`NoomApp/ContentView.swift`** — minimal sign-in form; exercises the `signIn` RPC path through gRPC-Core.
