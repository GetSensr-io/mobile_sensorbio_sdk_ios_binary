...
class NoomBandStateTests(unittest.TestCase):
    ...
    def test_user_facing_device_identity_uses_device_id_and_serial_number(self):
        ...
        self.assertNotIn("selectedDevice.id == connectedDeviceID", PAIR)
        self.assertIn("PairingAttemptCorrelation.canonicalDeviceID(", PAIR)
        self.assertIn("identity.connectionEstablished(", PAIR)
        self.assertIn("deviceID: canonicalDeviceID", PAIR)
